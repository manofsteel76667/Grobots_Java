...
public abstract class GBView extends JPanel {
	...
	protected Rectangle CalcExternalRect(Rectangle r) {
		return new Rectangle(r.left + bounds.left, r.top + bounds.top, r.right
				+ bounds.left, r.bottom + bounds.top);
	}
	...
	protected void DrawArc(Rectangle where, int startAngle, int length,
		...
		graphics.DrawArc(CalcExternalRect(where), startAngle, length, color,
				thickness);
	}

	protected void DrawStringLeft(String str, int x, int y, int size,
			Color color, boolean useBold) {
		graphics.DrawStringLeft(str, x + bounds.left, y + bounds.top, size,
				color, useBold);
	}

	protected void DrawStringRight(String str, int x, int y, int size,
			Color color, boolean useBold) {
		graphics.DrawStringRight(str, x + bounds.left, y + bounds.top, size,
				color, useBold);
	}

	protected void DrawStringCentered(String str, int x, int y, int size,
			Color color, boolean useBold) {
		graphics.DrawStringCentered(str, x + bounds.left, y + bounds.top, size,
				color, useBold);
	}

	protected void DrawStringPair(String str1, String str2, int left,
			int right, int y, int size, Color color, boolean useBold) {
		DrawStringLeft(str1, left, y, size, color, useBold);
		DrawStringRight(str2, right, y, size, color, useBold);
	}

	protected void DrawLongLeft(int n, int x, int y, int size, Color color,
			boolean useBold) {
		DrawStringLeft(n.toString(), x, y, size, color, useBold);
	}

	protected void DrawLongRight(int n, int x, int y, int size, Color color,
			boolean useBold) {
		DrawStringRight(n.toString(), x, y, size, color, useBold);
	}

	protected void DrawStringLongPair(String str, int n, int left, int right,
			int y, int size, Color color, boolean useBold) {
	...
	public boolean NeedsRedraw(boolean running) {
		...
		return InstantChanges()
				|| (!running || System.currentTimeMillis() >= lastDrawn
						+ interval) && DelayedChanges();
	}
