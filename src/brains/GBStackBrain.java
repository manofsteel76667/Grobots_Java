...
enum TwoVectortoVectorOperator {add,subtract,projection,rotateto,rotatefrom}
enum TwoVectortoScalarOperator{cross,dot}
enum TwoNumbertoNumberOperator{mod,rem}
...
// status
	...
	void WriteVectorVariable( int index,  FinePoint value) throws GBBadSymbolIndexError {
		if ( index < 0 || index >= spec.NumVectorVariables() )
			throw new GBBadSymbolIndexError();
		vectorVariables[index] = value;
	}

	void ExecuteInstruction(int ins, GBRobot robot, GBWorld world) throws GBUnknownInstructionError, GBStackOverflowError, GBBadSymbolIndexError, GBStackUnderflowError, GBReadOnlyError, GBUnknownHardwareVariableError, GBNotIntegerError, GBBadArgumentError, GBBadAddressError, GBOutOfMemoryError, GBGenericError, GBAbort {
	...
	void TwoNumberToNumberOp(TwoNumbertoNumberOperator op) throws GBStackUnderflowError {
		if ( stackHeight < 2 )
			throw new GBStackUnderflowError();
		-- stackHeight;
		double x = stack[stackHeight - 1];//TODO: check that this is in the right order
 ...
 */
		case mod: x = x - Math.floor(x / divisor) * divisor;
			break;
		case rem: x = x - (int)(x / divisor) * divisor;;
			break;
			default:
				break;
		}
...
/*
	void VectorToVectorOp(VectortoVectorOperator op) {
		FinePoint v = PopVector();
		PushVector((v.*op)());
	}
/*
	void VectorToScalarOp(VectortoScalarOperator op) {
		FinePoint v = PopVector();
		Push((v.*op)());
	}
*/
	void TwoVectorToVectorOp(TwoVectortoVectorOperator op) throws GBStackOverflowError, GBStackUnderflowError {
		FinePoint v2 = PopVector();
		FinePoint v1 = PopVector();
		FinePoint result=new FinePoint();
		switch(op){
		case add: result = v1.add(v2);
			break;
		case projection: result=v1.projection(v2);
			break;
		case subtract: result=v1.subtract(v2);
			break;
		default:
			break;		
		}
	...
	void TwoVectorToScalarOp(TwoVectortoScalarOperator op) throws GBStackOverflowError, GBStackUnderflowError {
		FinePoint v2 = PopVector();
		FinePoint v1 = PopVector();
		double result=0;
		switch(op){
		case cross:result=v1.cross(v2);
			break;
		case dot:result=v1.dotProduct(v2);
			break;
		default:
			break;		
		}
	...
	double Peek(int delta) throws GBBadArgumentError, GBStackUnderflowError, GBStackOverflowError {
		if ( delta < 1 )
	...
	double ReadLocalMemory(int addr, GBRobot robot) throws GBIndexOutOfRangeError {
		if ( addr < 1 || addr > robot.hardware.Memory() ) throw new GBIndexOutOfRangeError();
		if ( memory==null ) return 0;
		return memory[addr - 1];
	}

	void WriteLocalMemory(int addr, double val, GBRobot robot) throws GBIndexOutOfRangeError, GBOutOfMemoryError {
		if ( addr < 1 || addr > robot.hardware.Memory() ) throw new GBIndexOutOfRangeError();
		if ( memory==null ) {
	...
	void BrainError(GBError err, GBRobot robot, GBWorld world) throws GBAbort  {
		if ( world.reportErrors )
			GBError.NonfatalError(robot.Description() + " had error in brain, probably at "
				+ AddressAndLine(pc - 1) + ": " + err.ToString());
		status = BrainStatus.bsError;
	}

	public GBStackBrain( GBStackBrainSpec spc) throws GBBadSymbolIndexError, GBOutOfMemoryError{
		spec=spc;
		pc=spc.StartAddress();
		...
		memory=null;
		lastPrint=null;
		if ( variables.length==0 || vectorVariables.length ==0 || stack.length==0 || returnStack.length==0 )
	...
	public void Think(GBRobot robot, GBWorld world) throws GBBadArgumentError, GBOutOfMemoryError, GBGenericError, GBAbort {
		try {
	...
	public void ThinkOne(GBRobot robot, GBWorld world) throws GBUnknownInstructionError, GBStackOverflowError, GBBadSymbolIndexError, GBStackUnderflowError, GBReadOnlyError, GBUnknownHardwareVariableError, GBNotIntegerError, GBBadArgumentError, GBBadAddressError, GBOutOfMemoryError, GBGenericError, GBAbort {
	...
	double ReadHardware(int index, GBRobot robot, GBWorld world) throws GBUnknownHardwareVariableError  {
		switch ( StackBrainOpcode.byID(index )) {
		// world
			case hvTime: return (double)world.CurrentFrame();
		...
		// robot sensor
			...
			case hvRobotSensorAngleFound: return (robot.hardware.sensor1.WhereFound().subtract(robot.Position()).angle()); // inconsistent: distance then or now?
			case hvRobotSensorRangeFound: return (robot.hardware.sensor1.WhereFound().subtract(robot.Position()).norm());
			...
			case hvRobotSensorReloadingFound: return robot.hardware.sensor1.Reloading() ? 1 : 0;
			case hvRobotSensorFlagFound: return robot.hardware.sensor1.Flag();
			case hvRobotSensorRangeOverall: return (robot.hardware.sensor1.WhereOverall().subtract(robot.Position()).norm());
			case hvRobotSensorAngleOverall: return (robot.hardware.sensor1.WhereOverall().subtract(robot.Position()).angle());
		...
		// food sensor
			...
			case hvFoodSensorAngleFound: return (robot.hardware.sensor2.WhereFound().subtract(robot.Position()).angle());
			case hvFoodSensorRangeFound: return (robot.hardware.sensor2.WhereFound().subtract(robot.Position()).norm());
			...
			case hvFoodSensorRangeOverall: return (robot.hardware.sensor2.WhereOverall().subtract(robot.Position()).norm());
			case hvFoodSensorAngleOverall: return (robot.hardware.sensor2.WhereOverall().subtract(robot.Position()).angle());
		...
		// shot sensor
			...
			case hvShotSensorAngleFound: return (robot.hardware.sensor3.WhereFound().subtract(robot.Position()).angle());
			case hvShotSensorRangeFound: return (robot.hardware.sensor3.WhereFound().subtract(robot.Position()).norm());
			...
			case hvShotSensorRangeOverall: return (robot.hardware.sensor3.WhereOverall().subtract(robot.Position()).norm());
			case hvShotSensorAngleOverall: return (robot.hardware.sensor3.WhereOverall().subtract(robot.Position()).angle());
		...
		// defenses
			...
			case hvLastHit: return robot.LastHit()!=null ? robot.LastHit().ID() : 0;
		// blaster
		...
		//
			default: throw new GBUnknownHardwareVariableError();
		}
	}

	void WriteHardware(int index, double value,
			GBRobot robot, GBWorld world) throws GBReadOnlyError, GBNotIntegerError, GBBadArgumentError, GBUnknownHardwareVariableError {
		switch ( StackBrainOpcode.byID(index) ) {
		...
		// constructor
			case hvConstructorType: {
				...
				robot.hardware.constructor.Start(tmpindex!=0 ? robot.Owner().GetType(tmpindex) : null, 0);
			} break;
			...
			case hvChildID:
				throw new GBReadOnlyError();
		// robot sensor
			case hvRobotSensorRange: case hvRobotSensorFiringCost: throw new GBReadOnlyError();
			...
			case hvRobotSensorSeesFriends: robot.hardware.sensor1.SetSeesFriendly(value!=0); break;
			case hvRobotSensorSeesEnemies: robot.hardware.sensor1.SetSeesEnemy(value!=0); break;
			...
			case hvRobotSensorNumResults: case hvRobotSensorMaxResults: throw new GBReadOnlyError();
		// food sensor
			case hvFoodSensorRange: case hvFoodSensorFiringCost: throw new GBReadOnlyError();
			...
			case hvFoodSensorNumResults: case hvFoodSensorMaxResults: throw new GBReadOnlyError();
		// shot sensor
			case hvShotSensorRange: case hvShotSensorFiringCost: throw new GBReadOnlyError();
			...
			case hvShotSensorSeesFriendly: robot.hardware.sensor3.SetSeesFriendly(value!=0); break;
			case hvShotSensorSeesEnemy: robot.hardware.sensor3.SetSeesEnemy(value!=0); break;
			...
			case hvShotSensorNumResults: case hvShotSensorMaxResults: throw new GBReadOnlyError();
		// defenses
		...
		// weapons
			...
			case hvForceFieldRadius: throw new GBReadOnlyError();
		//
			default: throw new GBUnknownHardwareVariableError();
		}
	}

	FinePoint ReadHardwareVector(int index, GBRobot robot, GBWorld world) throws GBUnknownHardwareVariableError {
		switch ( StackBrainOpcode.byID(index) ) {
		...
		// sensors
			...
			case hvvRobotSensorWhereRelative: return robot.hardware.sensor1.WhereFound().subtract(robot.Position());
			...
			case hvvFoodSensorWhereRelative: return robot.hardware.sensor2.WhereFound().subtract(robot.Position());
			...
			case hvvShotSensorWhereRelative: return robot.hardware.sensor3.WhereFound().subtract(robot.Position());
	...
	void WriteHardwareVector(int index, FinePoint value, GBRobot robot, GBWorld world) throws GBReadOnlyError, GBUnknownHardwareVariableError {
		switch ( StackBrainOpcode.byID(index) ) {
	...
	void ExecutePrimitive(int index, GBRobot robot, GBWorld world) throws GBStackUnderflowError, GBNotIntegerError, GBBadArgumentError, GBStackOverflowError, GBBadAddressError, GBOutOfMemoryError, GBGenericError, GBUnknownInstructionError, GBAbort {
		...
		// stack manipulation
			...
			case opDropN: {
				int n = PopInteger();
				if ( n > stackHeight ) throw new GBBadArgumentError();
				stackHeight -= n;
			} break;
			...
			case opDup: temp = Peek(1); Push(temp); break;
			case op2Dup: temp = Peek(2); temp2 = Peek(1); Push(temp); Push(temp2); break;
		...
		// branches
			...
			case opIfGo: temp = Pop(); if ( Pop()!=0 ) pc = ToAddress(temp); break;
			case opIfElseGo: temp = Pop(); temp2 = Pop();
				if ( Pop()!=0 ) pc = ToAddress(temp2); else pc = ToAddress(temp); break;
			case opIfCall: temp = Pop(); if ( Pop()!=0 ) ExecuteCall(ToAddress(temp)); break;
			case opIfElseCall: temp = Pop(); temp2 = Pop();
				if ( Pop()!=0 ) ExecuteCall(ToAddress(temp2)); else ExecuteCall(ToAddress(temp)); break;
			case opIfReturn: if ( Pop()!=0 ) pc = PopReturn(); break;
			case opNotIfGo: temp = Pop(); if ( Pop()==0 ) pc = ToAddress(temp); break;
			case opNotIfReturn: if ( Pop()==0 ) pc = PopReturn(); break;
			case opNotIfCall: temp = Pop(); if ( Pop()==0 ) ExecuteCall(ToAddress(temp)); break;
		// arithmetic
			...
			case opReciprocal: Push(1 / Pop()); break;
			case opMod: TwoNumberToNumberOp(TwoNumbertoNumberOperator.mod); break;
			case opRem: TwoNumberToNumberOp(TwoNumbertoNumberOperator.rem); break;
			case opSquare: Push(Math.pow(Pop(),2)); break;
			case opSqrt: Push(Math.sqrt(Pop())); break;
			case opExponent: temp = Pop(); Push(Math.pow(Pop(), temp)); break;
			case opIsInteger: temp=Pop();Pushboolean(temp==Math.floor(temp)); break;
			case opFloor: Push(Math.floor(Pop())); break;
			case opCeiling: Push(Math.ceil(Pop())); break;
			case opRound: Push(Math.round(Pop())); break;
			case opMin: Push(Math.min(Pop(), Pop())); break;
			case opMax: Push(Math.max(Pop(), Pop())); break;
			case opAbs: Push(Math.abs(Pop())); break;
			case opSignum: Push(Math.signum(Pop())); break;
			case opReorient: Push(GBMath.reorient(Pop())); break;
			case opSine: Push(Math.sin(Pop())); break;
			case opCosine: Push(Math.cos(Pop())); break;
			case opTangent: Push(Math.tan(Pop())); break;
			case opArcSine: Push(Math.asin(Pop())); break;
			case opArcCosine: Push(Math.acos(Pop())); break;
			case opArcTangent: Push(Math.atan(Pop())); break;
			case opRandom: temp = Pop(); Push(world.Randoms().InRange(Pop(), temp)); break;
			case opRandomAngle: Push(world.Randoms().Angle()); break;
			case opRandomInt: temp = Pop(); Push(world.Randoms().intInRange((int)Math.ceil(Pop()), (int)Math.floor(temp))); break;
			case opRandomBoolean: Pushboolean(world.Randoms().bool(Pop())); break;
		// constants
			case opPi: Push(GBMath.kPi); break;
			case op2Pi: Push(GBMath.kPi * 2); break;
			case opPiOver2: Push(GBMath.kPi / 2); break;
			case opE: Push(GBMath.kE); break;
			case opEpsilon: Push(GBMath.kEpsilon); break;
			case opInfinity: Push(GBMath.kInfinity); break;
		// vector operations
			case opRectToPolar: { FinePoint v = PopVector(); Push(v.norm()); Push(v.angle()); } break;
			case opPolarToRect: temp = Pop(); temp2 = Pop(); PushVector(FinePoint.makePolar(temp2, temp)); break;
			case opVectorAdd: TwoVectorToVectorOp(TwoVectortoVectorOperator.add); break;
			case opVectorSubtract: TwoVectorToVectorOp(TwoVectortoVectorOperator.subtract); break;
			case opVectorNegate: PushVector(PopVector().negate()); break;
			case opVectorScalarMultiply: temp = Pop(); PushVector(PopVector().multiply(temp)); break;
			case opVectorScalarDivide: temp = Pop(); PushVector(PopVector().divide(temp)); break;
			case opVectorNorm: Push(PopVector().norm()); break;
			case opVectorAngle: Push(PopVector().angle()); break;
			case opDotProduct: TwoVectorToScalarOp(TwoVectortoScalarOperator.dot);  break;
			case opProject: TwoVectorToVectorOp(TwoVectortoVectorOperator.projection); break;
			case opCross: TwoVectorToScalarOp(TwoVectortoScalarOperator.cross); break;
			case opUnitize: PushVector(PopVector().unit()); break;
			case opVectorRotateTo: TwoVectorToVectorOp(TwoVectortoVectorOperator.rotateto); break;
			case opVectorRotateFrom: TwoVectorToVectorOp(TwoVectortoVectorOperator.rotatefrom); break;
			case opDistance: Push(PopVector().subtract(PopVector()).norm()); break;
		...
		// booleans
			case opNot: Pushboolean( Pop()==0); break;
			case opAnd: temp = Pop(); temp2 = Pop(); Pushboolean(temp!=0 && temp2!=0); break;
			case opOr: temp = Pop(); temp2 = Pop(); Pushboolean(temp!=0 || temp2!=0); break;
			case opXor: temp = Pop(); temp2 = Pop();
				Pushboolean(temp!=0 && temp2==0 || temp==0 && temp2!=0); break;
			case opNand: temp = Pop(); temp2 = Pop(); Pushboolean(! (temp!=0 && temp2!=0)); break;
			case opNor: temp = Pop(); temp2 = Pop(); Pushboolean(! (temp!=0 || temp2!=0)); break;
			case opValueConditional: temp = Pop(); temp2 = Pop();
				if ( Pop()!=0 ) Push(temp2); else Push(temp);
				break;
		// misc external
			case opPrint:
				DoPrint(Double.toString(Pop()));
				if ( world.reportPrints )
					GBError.NonfatalError(robot.Description() + " prints: " + lastPrint);
				break;
			case opPrintVector:
				...
				if ( world.reportPrints )
					GBError.NonfatalError(robot.Description() + " prints: " + lastPrint);
				break;
			case opBeep: /*TODO: waiting for sound StartSound(siBeep);*/ break;
			case opStop: status=BrainStatus.bsStopped; break;
			case opPause: if ( world.reportErrors ) world.running = false; break;
			case opSync: remaining = 0; break;
		// basic hardware
			case opSeekLocation: robot.EngineSeek(PopVector(), new FinePoint(0, 0)); break;
			case opSeekMovingLocation: {
				FinePoint vel = PopVector();
				robot.EngineSeek(PopVector(), vel);
				} break;
			case opDie: robot.Die(robot.Owner()); status=BrainStatus.bsStopped; break;
			...
			case opSendMessage: {
					...
					if ( numArgs <= 0 )
						throw new GBGenericError("Cannot send message of non-positive length");
					robot.hardware.radio.Send(sendee, tempInt, robot.Owner());
				} break;
			case opReceiveMessage: {
					tempInt = PopInteger();
					GBMessage received = robot.hardware.radio.Receive(tempInt, robot.Owner());
					if ( received == null ) {
						Push(0);
					} else {
						if ( received.length <= 0 ) {
							throw new GBGenericError("non-positive length message received");
						}
			...
			case opAutoConstruct: {
				...
				if ( robot.Energy() > robot.hardware.MaxEnergy() * .9 ) {
					if ( ctor.Type()==null ) ctor.Start(robot.Type(),0);
					ctor.SetRate(ctor.MaxRate());
				} else
					ctor.SetRate(ctor.Type()!=null && robot.Energy() > ctor.Remaining() + 10 ? ctor.MaxRate() : 0);
				} break;
			case opBalanceTypes: { // frac type --
					RobotType theType = robot.Owner().GetType(PopInteger());
					double fraction = Pop();
					if (theType!=null && theType.population < fraction * robot.Owner().Scores().Population())
						robot.hardware.constructor.Start(theType,0); //FIXME don't abort?
				} break;
		// sensors
		...
		// weapons
			...
			case opLeadBlaster: { //pos vel --
				...
				if ( target.isNonzero() && target.norm() <= robot.hardware.blaster.MaxRange() + robot.Radius() )
			...
			case opLeadGrenade: { //pos vel --
				...
				if ( target.isNonzero() && target.norm() <= robot.hardware.grenades.MaxRange() + robot.Radius() )
	...
	static FinePoint LeadShot(FinePoint pos, FinePoint vel, double shotSpeed, double r) {
	if (true){
		double dt = (pos.add(vel.multiply(pos.norm() / shotSpeed))).norm() / shotSpeed; //two plies for accuracy with radially moving targets
	...
	void FirePeriodic(GBSensorState sensor, GBWorld world) throws GBStackUnderflowError, GBStackOverflowError, GBNotIntegerError {
